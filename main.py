...
import db  # SQLite DB helper module


# Initialize DB, create tables, example users
db.init_db()
db.add_user('user1', 'pass1')  # Remove or disable in production
db.add_user('user2', 'pass2')
...
def main():
    ...
    def initial_options():
        st.session_state.user_menu = streamlit_option_menu.option_menu(
            ...
            options=[
                ...
                'Describe me a movie',
                'Check all Movies',
                'Recommend based on my interest'
            ],
            icons=['film', 'film', 'film', 'heart'],
            menu_icon='list',
        ...
        if st.session_state.user_menu == 'Recommend me a similar movie':
            ...
            paging_movies()
        elif st.session_state.user_menu == 'Recommend based on my interest':
            recommend_based_on_interest()
    ...
    def recommend_display():
        ...
        if rec_button:
            db.add_to_history(st.session_state['username'], selected_movie_name)
    ...
    def recommend_based_on_interest():
        st.title("Recommendations Based On Your Interest")
        history = db.get_user_history(st.session_state['username'])
        if len(history) == 0:
            st.warning("No search history found. Please select movies in 'Recommend me a similar movie' tab first.")
            return
        ...
        for movie_name in history:
            movies, posters = preprocess.recommend(new_df, movie_name, r'Files/similarity_tags_tags.pkl')
            for i, m in enumerate(movies):
                if m not in displayed_local:
                    all_recommendations.append(m)
                    all_posters.append(posters[i])
                    displayed_local.append(m)
        ...
        st.subheader("Top recommendations based on your interests:")
        cols = st.columns(min(5, len(all_recommendations)))
        for idx, col in enumerate(cols):
            col.text(all_recommendations[idx])
            col.image(all_posters[idx])
